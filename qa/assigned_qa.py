...
##  GAIA point source.
##  is_gps     = sv_mtl['GAIA_POINTSOURCE']
  
##
colors     = plt.rcParams['axes.prop_cycle'].by_key()['color']
  
##  Source types.
morphtypes = set(assigned['MORPHTYPE'])

##
rmag       = 22.5 - 2.5 * np.log10(assigned['FLUX_R'].quantity / assigned['MW_TRANSMISSION_R'].quantity)
rfibmag    = 22.5 - 2.5 * np.log10(assigned['FIBERFLUX_R'].quantity / assigned['MW_TRANSMISSION_R'].quantity)

for mtype, color in zip(morphtypes, colors):
  isin     = assigned['MORPHTYPE'] == mtype
  
  pl.plot(rmag[isin], rfibmag[isin], '.', markersize=1, c=color, label=mtype+'-{}'.format(len(assigned[assigned['MORPHTYPE'] == mtype])))   
  
  print(mtype, len(assigned[assigned['MORPHTYPE'] == mtype]))
...
##  pl.show()
pl.savefig('plots/assigned_fibmag.png')
pl.clf()

##  (z - W1) - (3. / 2.5) * (g - r) + 1.2;
gmag  = 22.5 - 2.5 * np.log10(assigned['FLUX_G'].quantity  / assigned['MW_TRANSMISSION_G'].quantity) 
zmag  = 22.5 - 2.5 * np.log10(assigned['FLUX_Z'].quantity  / assigned['MW_TRANSMISSION_Z'].quantity)
w1mag = 22.5 - 2.5 * np.log10(assigned['FLUX_W1'].quantity / assigned['MW_TRANSMISSION_W1'].quantity)
