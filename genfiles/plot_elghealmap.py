...
def read_elgs(mtype, snr):
  ##  [hpind, hpra, hpdec, tdensity].
  return  np.load('/global/cscratch1/sd/mjwilson/BGS/SV-ASSIGN/healmaps/elg_tdensity_{}_{:d}_{:d}.npy'.format(mtype, snr, nside))
...
if __name__ == '__main__':    
  ...
  fig, axarr = plt.subplots(nrows=5, ncols=4, figsize=(15, 15))
  ...
  for i, mtype in enumerate(mtypes):
    for j, snr in enumerate(snrs):
      ...
      axarr[i][j].set_title(r'{} ELG (SNR $\geq {})$'.format(mtype, snr))
      axarr[i][j].set_xlim(360., 0.)

  pl.savefig('skydepths/elgs.png')
