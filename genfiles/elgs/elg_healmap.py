...
cols           = ['RA', 'DEC', 'MORPHTYPE', 'DESI_TARGET', 'FLUX_G', 'FLUX_R', 'FLUX_Z',\
                  ...
                  'FLUX_IVAR_G', 'FLUX_IVAR_R', 'FLUX_IVAR_Z', 'TARGETID']

def write_elgs(elgs, mtype=None, snr=None, write_elgs=False, ext=''):  
  if write_elgs:
...
for mtype in mtypes:
  for snr in snrs:
    toprint = elgs[(elgs['MORPHTYPE'] == mtype) & (elgs['RSNR'] >= snr) & (elgs['ZSNR'] >= snr)]
    write_elgs(toprint, write_elgs=False, mtype=mtype, snr=snr, ext=ext)
